...
def load_road_data(city_name: str) -> pd.DataFrame:
    ...
    print(
        f"  • Geographic bounds: {bounds['lon_min']:.3f} to {bounds['lon_max']:.3f} lng, {bounds['lat_min']:.3f} to {bounds['lat_max']:.3f} lat"
    )
    ...
    for method in methods:
        ...
        try:
            cluster_result = allocator.cluster(
                sample_points, n_clusters=n_clusters, distance=method, random_state=42
            )
...
def routing_analysis(points: pd.DataFrame, city_name: str) -> dict:
    ...
    try:
        ...
        for _i, (_, stop) in enumerate(route_order.iterrows()):
...
def assignment_analysis(points: pd.DataFrame, city_name: str) -> dict:
    ...
    print("Service depot locations:")
    for _, depot in depots.iterrows():
        print(
            f"  • {depot['depot_name']}: ({depot['longitude']:.3f}, {depot['latitude']:.3f}) - Capacity: {depot['capacity']}"
        )
    ...
    try:
        ...
        for depot_id in assignments.index:
            assigned_points = assignment_result.data[
                assignment_result.data["assigned_worker"] == depot_id
            ]
            ...
            for _, point in assigned_points.iterrows():
                ...
                distance = allocator.get_distance_matrix(
                    point_coords, depot_coords, method="haversine"
                )[0, 0]
        ...
        return {
            ...
            "depot_utilization": {
                depots.iloc[i]["depot_name"]: (assignments.get(i, 0) / depots.iloc[i]["capacity"])
                * 100
...
def distance_matrix_analysis(points: pd.DataFrame, city_name: str) -> dict:
    ...
    for method in methods:
        try:
            ...
            results[method] = {"distances": distances, "statistics": stats, "elapsed_time": elapsed}
...
def main():
    ...
    print("  • Urban planning: Road network analysis and zone creation")
    print("  • Logistics: Delivery route optimization and depot placement")
    print("  • Emergency services: Response zone assignments")
    print("  • Maintenance: Infrastructure inspection scheduling")
    print("  • Transportation: Public transit route planning")

    print("\n✨ Key Features Showcased:")
    print("  • Real geographic data processing")
    print("  • Multiple clustering algorithms")
    print("  • TSP route optimization")
    print("  • Distance-based assignments")
    print("  • Multiple distance calculation methods")
    print("  • Performance metrics and timing")

    print("\n🎉 Real-world workflow demonstration complete!")
    print("   Production-ready results using actual OpenStreetMap data")
