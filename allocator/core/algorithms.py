...
def kahip_cluster(data: pd.DataFrame | np.ndarray, n_clusters: int,
                  distance_method: str = 'euclidean', n_closest: int = 15,
                  seed: int | None = None, balance_edges: bool = False,
                  buffoon: bool = False, kahip_dir: str = './KaHIP/src',
...
def _run_kahip_buffoon(G: nx.Graph, n_clusters: int, seed: int, kahip_dir: str) -> np.ndarray:
    ...
    buffoon_cmd = (f'mpirun -n {n_clusters} {kahip_dir}/optimized/buffoon metis.graph '
    ...
    # Execute command
    p = subprocess.Popen(shlex.split(buffoon_cmd), stdout=subprocess.PIPE, 
...
def _run_kahip_wrapper(G: nx.Graph, n_clusters: int, seed: int, balance_edges: bool) -> np.ndarray:
    ...
    try:
        ...
        raise ImportError("kahipwrapper package is required for KaHIP clustering. "
    ...
    if balance_edges:
        edgecut, part = kaHIP.kaffpa_balance_NE(ncount, vwgt, xadj, adjcwgt, adjncy,
                                               nparts, imbalance, suppress_output,
                                               seed, mode)
    else:
        edgecut, part = kaHIP.kaffpa(ncount, vwgt, xadj, adjcwgt, adjncy, nparts,
