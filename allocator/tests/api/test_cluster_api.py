...
class TestClusterAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.alt_data = pd.DataFrame({
            'start_long': [101.0, 101.1, 101.2, 101.3, 101.0, 101.1],
            'start_lat': [13.0, 13.1, 13.2, 13.3, 13.0, 13.1],
            'segment_id': range(6)
        })
    ...
    def test_kmeans_with_alternative_columns(self):
        """Test K-means with alternative column names."""
        result = kmeans(self.alt_data, n_clusters=2, random_state=42)
    ...
    def test_kmeans_different_distances(self):
        ...
        for distance in ['euclidean', 'haversine']:
            with self.subTest(distance=distance):
                result = kmeans(self.modern_data, n_clusters=2, 
                              distance=distance, random_state=42)
                self.assertTrue(result.converged)
    ...
    def test_cluster_metadata(self):
        ...
        result = kmeans(self.modern_data, n_clusters=3, distance='haversine', 
