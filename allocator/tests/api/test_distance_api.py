...
class TestDistanceAPI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Alternative column format data
        self.alt_points = pd.DataFrame({
    ...
    def test_distance_assignment_alternative_columns(self):
        """Test distance assignment with alternative column names."""
        alt_centers = pd.DataFrame({
            'start_long': [101.05, 101.25],
            'start_lat': [13.05, 13.25],
            'center_id': ['C1', 'C2']
        })
        
        result = distance_assignment(self.alt_points, alt_centers, distance='euclidean')
        
        self.assertIsInstance(result, SortResult)
        self.assertEqual(len(result.data), len(self.alt_points))
        self.assertIn('assigned_worker', result.data.columns)
    ...
    def test_distance_assignment_numpy_arrays(self):
        ...
        self.assertTrue(all(0 <= assignment < len(self.array_centers) for assignment in assignments))
    ...
    def test_empty_data_handling(self):
        ...
        with self.assertRaises((ValueError, IndexError)):
            distance_assignment(empty_points, self.centers)
