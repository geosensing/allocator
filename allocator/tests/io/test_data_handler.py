...
class TestDataHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_data_legacy = pd.DataFrame({
            'start_long': [101.0, 101.1, 101.2],
            'start_lat': [13.0, 13.1, 13.2],
            'segment_id': [1, 2, 3]
        })
        
        self.numpy_data = np.array([
            [101.0, 13.0],
            [101.1, 13.1],
            [101.2, 13.2]
        ])
    ...
    def test_load_dataframe_legacy_columns(self):
        """Test loading DataFrame with legacy column names."""
        result = DataHandler.load_data(self.test_data_legacy)
        
        self.assertIsInstance(result, pd.DataFrame)
        # Should map legacy columns to modern ones
        self.assertIn('longitude', result.columns)
        self.assertIn('latitude', result.columns)
        
        # Check that values are correctly mapped
        expected_lons = self.test_data_legacy['start_long'].values
        expected_lats = self.test_data_legacy['start_lat'].values
        np.testing.assert_array_equal(result['longitude'].values, expected_lons)
        np.testing.assert_array_equal(result['latitude'].values, expected_lats)
    ...
    def test_load_json_file(self):
        ...
        json_data = {
            'features': [
                {
                    'geometry': {'coordinates': [101.0, 13.0]},
                    'properties': {'id': 1}
                },
                {
                    'geometry': {'coordinates': [101.1, 13.1]},
                    'properties': {'id': 2}
                }
            ]
        }
    ...
    def test_column_mapping(self):
        ...
        test_cases = [
            # (input_columns, expected_modern_columns)
            (['start_long', 'start_lat'], ['longitude', 'latitude']),
            (['end_long', 'end_lat'], ['longitude', 'latitude']),
            (['lon', 'lat'], ['longitude', 'latitude']),
            (['lng', 'lat'], ['longitude', 'latitude']),
        ]
        
        for input_cols, expected_cols in test_cases:
            with self.subTest(input_cols=input_cols):
                test_df = pd.DataFrame({
                    input_cols[0]: [101.0, 101.1],
                    input_cols[1]: [13.0, 13.1],
                    'id': [1, 2]
                })
    ...
    def test_numpy_array_wrong_shape(self):
        ...
        # Too many columns (should work but only use first 2)
        wide_array = np.array([[101.0, 13.0, 1], [101.1, 13.1, 2]])
        result = DataHandler.load_data(wide_array)
        
        self.assertEqual(result.shape[1], 2)  # Should only have lon, lat columns
