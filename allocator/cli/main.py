...
@click.option('--workers', '-w', type=click.Path(exists=True), 
              help='Worker locations file')
@click.option('--by-worker', is_flag=True, 
              help='Sort points by worker instead of workers by point')
...
@click.option('--algorithms', '-a', default='kmeans,kahip',
    ...
    try:
        ...
        for algo in algos:
            if algo in ['kmeans', 'kahip']:
                ...
                result = cluster(input_file, n_clusters=n_clusters, 
                               method=algo, distance=distance)
                results[algo] = result
            else:
