...
def cluster(data: str | pd.DataFrame | np.ndarray | list,
           n_clusters: int = 3,
           method: str = 'kmeans',
    ...
    if method == 'kmeans':
        return kmeans(df, n_clusters=n_clusters, distance=distance, 
                     random_state=random_state, **kwargs)
    elif method == 'kahip':
        return kahip(df, n_clusters=n_clusters, distance=distance,
                    random_state=random_state, **kwargs)
    else:
        raise ValueError(f"Unknown clustering method: {method}")


def kmeans(data: pd.DataFrame | np.ndarray | list,
          n_clusters: int = 3,
          distance: str = 'euclidean',
          max_iter: int = 300,
          random_state: int | None = None,
    ...
    if distance == 'euclidean':
        ...
        coords = df[['longitude', 'latitude']].values if 'longitude' in df.columns else df[['start_long', 'start_lat']].values
        distances = euclidean_distance_matrix(coords, result['centroids'])
        inertia = np.sum([distances[i, result['labels'][i]]**2 for i in range(len(result['labels']))])
...
def kahip(data: pd.DataFrame | np.ndarray,
         n_clusters: int = 3,
         distance: str = 'euclidean', 
         n_closest: int = 15,
