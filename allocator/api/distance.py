...
def sort_by_distance(points: str | pd.DataFrame | np.ndarray | list,
...
def _sort_workers_by_point(points_df: pd.DataFrame, 
...
def _sort_points_by_worker(points_df: pd.DataFrame,
                          workers_df: pd.DataFrame, 
                          distance: str = 'euclidean',
                          **kwargs) -> SortResult:
...
def distance_assignment(points: str | pd.DataFrame | np.ndarray | list,
                       workers: str | pd.DataFrame | np.ndarray | list,
                       distance: str = 'euclidean',
                       **kwargs) -> SortResult:
    """
...
def assign_to_closest(points: str | pd.DataFrame | np.ndarray | list,
                     workers: str | pd.DataFrame | np.ndarray | list,
                     distance: str = 'euclidean',
                     **kwargs) -> SortResult:
    """
    ...
    labels = sort_by_distance_assignment(
        points_df, workers_coords, 
        distance_method=distance, **kwargs
    )
...
def _extract_coordinates(df: pd.DataFrame) -> np.ndarray:
    ...
    if 'longitude' in df.columns and 'latitude' in df.columns:
        ...
        return df[['start_long', 'start_lat']].values
    else:
        raise ValueError("DataFrame must contain coordinate columns")